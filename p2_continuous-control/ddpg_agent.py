...
class Agent():
    ...
    def __init__(self, state_size, action_size, hyper_params, num_agents, random_seed):
        ...
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.hyper_params[4], weight_decay=self.hyper_params[6])

        # Critic Network (w/ Target Network)
        self.critic_local = Critic(state_size, action_size, random_seed).to(device)
        self.critic_target = Critic(state_size, action_size, random_seed).to(device)
        self.critic_optimizer = optim.Adam(self.critic_local.parameters(), lr=self.hyper_params[5], weight_decay=self.hyper_params[7])
        ...
        self.eps = self.hyper_params[8]

        # Replay memory
        self.memory = ReplayBuffer(action_size, self.hyper_params[0], self.hyper_params[1], random_seed)
    ...
    def step(self, states, actions, rewards, next_states, dones):
        ...
        if len(self.memory) > self.hyper_params[1]:
            experiences = self.memory.sample()
            self.learn(experiences, self.hyper_params[2])
    ...
    def learn(self, experiences, gamma):
        ...
        self.soft_update(self.critic_local, self.critic_target, self.hyper_params[3])
        self.soft_update(self.actor_local, self.actor_target, self.hyper_params[3])                     
